...
makestatus=run_command(f'cd {pld_base_dir}/compiler; make -f test_PR.mk --question ifcc')
if makestatus: # updates are needed
    makestatus=run_command(f'cd {pld_base_dir}/compiler; make -f test_PR.mk ifcc',toscreen=True) # this time we run `make` for real
