...
def run_command(string, logfile=None, toscreen=False):
    ...
    assert process.returncode != None # sanity check
    if logfile:
...
# use to invoke ifcc if in multiple-files mode
if args.wrapper:
    wrapper=os.path.realpath(os.getcwd()+"/"+ args.wrapper)
else:
    wrapper=os.path.dirname(os.path.realpath(__file__))+"/ifcc-wrapper.sh"

if not os.path.isfile(wrapper) and not (args.S or args.c or args.output):
...
for jobname in jobs:
    ...
    if "NOT_IMPLEMENTED" in jobname:
        continue
        
    print('TEST-CASE: '+jobname)
    os.chdir(f'{pld_base_dir}/ifcc-test-output/{jobname}')
    ...
    ## IFCC compiler
    if args.wrapper:
        ifccstatus=run_command(f'{wrapper} asm-ifcc.s input.c', "ifcc-compile.txt")
    else:
        ifccstatus=run_command(f'{pld_base_dir}/compiler/ifcc input.c > asm-ifcc.s', 'ifcc-compile.txt')
    
    if (gccstatus != 0 and ifccstatus != 0) or "FAILTEST" in jobname:
        print("TEST OK")
        nb_test_valid += 1
        continue
...
print(f"Tests passed: {nb_test_valid}/{len(jobs)}")